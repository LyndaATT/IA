...
public class PerceptronMultiNewLabel {
 ...
 /** OneHot :
  * @param etiquette : entier, qui est l'étiquette de notre classe
 ...
 public static float[][] InitialiseW(int D) {
     float[][] poidInitial = new float[27][D];
     for(int j = 0; j<27 ; j++) {
         for (int i = 0; i < D; i++) {
  ...
  *
  */
 public static void miseAJour(float[] donnee, float[][] poids, float eta, int label){
     float[] yl = InfPerception(donnee, poids);
     int[] OH = OneHot(label);
     for (int l = 0; l < poids.length; l++) {
         for (int j = 0; j < poids[l].length; j++) {
             poids[l][j] -= donnee[j]*eta*(yl[l] - OH[l]);
         }
     }
 }
 ...
 public static int argmax(float[] tab) {
   ...
   */
 public static int epoqueApprentissage(float[][] donnee, float[][] poids, int[] label,float eta){
     int nombreErreur = 0;
     for (int m = 0; m < donnee.length; m++) {
         float[] tableauPropab = InfPerception(donnee[m], poids);
         int indiceMax = argmax(tableauPropab);
         miseAJour(donnee[m],poids,eta,label[m]);
