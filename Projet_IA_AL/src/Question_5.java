    ...
    //  Apprentissage
     ...
     // Ainsi si on veut aceder a l'image m de validData il suffira de faire db.getImage(i2+m+1)
        ...
        System.out.println("Nombre d'erreur dans la phase de validation  : " + nbErreurValidation);
        	...
        	// Question 5 - les 5 images bien classées mais avec les pires scores d'inférences:
        DessinerImage imageS = new DessinerImage();
        int irna =0;
        for (int m = 0; m < Nv; m++) {
            float[] probaDonnee = PerceptronMulti.InfPerception(validData[m], poid);
            int ind = PerceptronMulti.argmax(probaDonnee);
            if(ind == validLabel[m]){
                imageS.add(m,probaDonnee[ind]);
            }
        }
        int[] tabInd = imageS.indiceMin();
        System.out.println("Indice des moins bien classées = "+Arrays.toString(tabInd));

        for (int img : tabInd) {
            sauvegarderImage(sonIndiceDansDB[img+1], Integer.toString(img+10));
        }
	}
}
